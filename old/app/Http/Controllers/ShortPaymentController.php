...
class ShortPaymentController extends Controller
{
    ...
    public function generatePaymentUrl()
    {
        ...
        $returnUrl = 'https://admissions.sau.ac.in/index.php/short_term_payment/success'; // Your success URL
        $mandatoryFields = $referenceNo . '|' . $submerchantId . '|' . $transactionAmount . '|' . $name . '|' . $email . '|' . $mobile; // Custom fields, like customer details
        $optionalFields = $userId . '|shortterm-coursefee|x|x|x|x|x'; // Optional fields
    ...
    public function paymentResponse(Request $request)
    {
        ...
        if ($Response_Code == 'E000') {
            ...
            return redirect('short_term_application_fee')->with('success', 'Your Payment has been successfully completed');
        } else {
            ...
            return redirect('short_term_application_fee')->with('warning', 'Please complete payment process for registration');
        }
    }
}
